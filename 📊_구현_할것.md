...
```python
# AI 학명 수정 모듈 예시
class AISpeciesNameCorrector:
    def __init__(self):
        self.gemini_client = GeminiAPI()
    
    def suggest_correction(self, input_name: str) -> dict:
        """AI 기반 학명 수정 제안"""
        prompt = f"""
        다음 학명의 오타나 형식 오류를 수정해주세요:
        입력: {input_name}
        
        응답 형식:
        - corrected_name: 수정된 학명
    ...
    def validate_binomial_format(self, name: str) -> bool:
        """이명법 형식 검증"""
        # Genus species 형식 확인
        pass
```
...
-- AI 학명 수정 이력 테이블
CREATE TABLE ai_correction_history (
    id SERIAL PRIMARY KEY,
